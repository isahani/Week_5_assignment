...
public class FirstNameUtility {
    ...
    public int encodeFirstName(String firstName, String middleInitial) throws MissingNameException {
        int middleInitialCode = 0;
        firstName = firstName.toUpperCase(Locale.ROOT);
        middleInitial = middleInitial.toUpperCase(Locale.ROOT);
        //code
        if (firstName.equals(null)){
            throw new MissingNameException(firstName);
        }
        if (middleInitialCodeMap.containsKey(middleInitial)){
            middleInitialCode = middleInitialCodeMap.get(middleInitial);
        }
        if (firstNameCodeMap.containsKey(firstName)) {
           int firstNamecode = firstNameCodeMap.get(firstName);
           return firstNamecode + middleInitialCode;
        }
        else {
          Character firstinitial = firstName.charAt(0);
          int firstInitialCode = firstInitialCodeMap.get(firstinitial);
          return firstInitialCode + middleInitialCode;
        }
